...
use orbit_core::{FileId, JobId, JobStatus, JobType, OrbitResult, SourceId, now_iso8601};
use rusqlite::params;
...
impl<'a> IndexJobRepository<'a> {
    ...
    pub fn set_status(&self, id: &JobId, status: JobStatus) -> OrbitResult<()> {
        let now = now_iso8601();
    ...
    pub fn list_queued(&self, limit: u32) -> OrbitResult<Vec<JobRecord>> {
        let conn = self.catalog.lock();
        let mut stmt = conn
            .prepare(
    ...
    pub fn count_by_status(&self) -> OrbitResult<Vec<(JobStatus, u64)>> {
        let conn = self.catalog.lock();
        let mut stmt = conn
