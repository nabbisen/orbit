...
use orbit_core::{OrbitError, OrbitResult, now_iso8601};
use rusqlite::params;
use serde::Serialize;
...
impl<'a> SettingsRepository<'a> {
    ...
    pub fn set<T: Serialize>(&self, key: &str, value: &T) -> OrbitResult<()> {
        let json = serde_json::to_string(value)
            .map_err(|e| OrbitError::Database(format!("settings serialize: {e}")))?;
    ...
    pub fn get<T: DeserializeOwned>(&self, key: &str) -> OrbitResult<Option<T>> {
        let conn = self.catalog.lock();
        ...
        match json {
            ...
            Some(json) => serde_json::from_str(&json)
                .map(Some)
                .map_err(|e| OrbitError::Database(format!("settings deserialize {key}: {e}"))),
        }
    }
}
