...
pub const CATALOG_FILE_NAME: &str = "orbit-catalog.sqlite3";

/// File name of the localcache-managed payload database (Appendix A §3).
pub const CACHE_FILE_NAME: &str = "orbit-cache.sqlite3";

/// The authoritative orbit catalog.
pub struct Catalog {
...
impl Catalog {
    ...
    /// §6.2).
    pub fn open(path: impl AsRef<Path>) -> OrbitResult<Self> {
    ...
    pub fn open_in_memory() -> OrbitResult<Self> {
        let conn = Connection::open_in_memory().map_err(db_err)?;
        Self::from_connection(conn, PathBuf::from(":memory:"))
    }

    fn from_connection(conn: Connection, path: PathBuf) -> OrbitResult<Self> {
    ...
    pub fn schema_version(&self) -> OrbitResult<i64> {
        let conn = self.lock();
        let version = conn
            .query_row(
...
/// Map a rusqlite error to the typed orbit error.
pub(crate) fn db_err(e: rusqlite::Error) -> OrbitError {
    OrbitError::Database(e.to_string())
}
