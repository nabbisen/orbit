...
use orbit_core::{ChunkId, FileId, OrbitError, OrbitResult, now_iso8601};
...
impl KeywordSearchEngine for Fts5KeywordEngine<'_> {
    fn index(&self, documents: &[KeywordDocument]) -> OrbitResult<()> {
    ...
    fn delete(&self, chunk_ids: &[ChunkId]) -> OrbitResult<()> {
        let mut conn = self.catalog.lock();
        let tx = conn.transaction().map_err(db)?;
    ...
    fn search(&self, query: &str, limit: u32) -> OrbitResult<Vec<KeywordCandidate>> {
...
fn db(e: rusqlite::Error) -> OrbitError {
