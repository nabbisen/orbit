...
use orbit_core::{FileId, FileStatus, OrbitError, OrbitResult, SourceId, now_iso8601};
...
impl<'a> FileRepository<'a> {
    ...
    pub fn insert(&self, new: NewFile) -> OrbitResult<FileRecord> {
    ...
    fn get_by_path_id(&self, id: &FileId) -> OrbitResult<FileRecord> {
        let conn = self.catalog.lock();
        let mut stmt = conn
        ...
        match rows.next() {
            ...
            None => Err(OrbitError::FileNotFound),
        }
    }

    /// Touch a file confirmed unchanged by the metadata check.
    pub fn touch_seen(&self, id: &FileId) -> OrbitResult<()> {
        let now = now_iso8601();
        let conn = self.catalog.lock();
        conn.execute(
    ...
    pub fn set_status(&self, id: &FileId, status: FileStatus) -> OrbitResult<()> {
        let conn = self.catalog.lock();
        conn.execute(
    ...
    pub fn mark_missing_unseen(&self, source_id: &SourceId, cutoff: &str) -> OrbitResult<u64> {
        let conn = self.catalog.lock();
        let n = conn
            .execute(
    ...
    pub fn count_by_status(&self, source_id: &SourceId) -> OrbitResult<Vec<(FileStatus, u64)>> {
        let conn = self.catalog.lock();
        let mut stmt = conn
            .prepare(
...
fn row_to_record(row: &Row<'_>) -> rusqlite::Result<OrbitResult<FileRecord>> {
