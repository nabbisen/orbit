...
use orbit_core::{
    FileStatus, JobType, OrbitResult, SourceId, now_iso8601, system_time_iso8601,
};
...
impl<'a> Scanner<'a> {
    ...
    pub fn scan(&self, request: &ScanRequest, cancel: &AtomicBool) -> OrbitResult<ScanSummary> {
        ...
        let source = sources
            ...
            .ok_or(orbit_core::OrbitError::SourceNotFound)?;
...
fn skip_component(policy: &CompiledPolicy, source: &SourceRecord, name: &str) -> bool {
    ...
    if CompiledPolicy::component_hidden(name)
        && source.hidden_file_policy == orbit_core::HiddenFilePolicy::Exclude
    {
        return true;
    }
    false
...
fn symlink_allowed(policy: &CompiledPolicy, root: &Path, path: &Path) -> bool {
    match policy.symlink_policy {
        orbit_core::SymlinkPolicy::Ignore => false,
        orbit_core::SymlinkPolicy::FollowWithinSource
        | orbit_core::SymlinkPolicy::FollowAllWithWarning => match std::fs::canonicalize(path) {
