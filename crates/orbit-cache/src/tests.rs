//! Tests for orbit-cache, validating Appendix A acceptance criteria:
...
use crate::{CacheService, EngineOptions, OrbitCacheNamespace};
use orbit_core::{CleanupAction, CleanupPlan, DataClass};
use orbit_db::{CACHE_FILE_NAME, CATALOG_FILE_NAME, Catalog};
use orbit_fs::ValidatedPath;
use orbit_core::SourceId;
...
// Appendix A §3: payloads live in orbit-cache.sqlite3, not the catalog.
#[test]
fn payloads_live_in_separate_database() {
    ...
    let engine = service
        .engine::<Segments>(
            &catalog,
            &OrbitCacheNamespace::ExtractSegments,
...
fn namespaces_are_distinct_and_classed() {
    let a = OrbitCacheNamespace::EmbeddingBundle {
        model_id: "model-a".into(),
        vector_format: "fp32".into(),
    };
    let b = OrbitCacheNamespace::EmbeddingBundle {
    ...
    assert_eq!(
        OrbitCacheNamespace::PreviewCache.data_class(),
...
fn usage_reports_entries_and_bytes() {
    ...
    let engine = service
        .engine::<Segments>(
            &catalog,
            &OrbitCacheNamespace::ChunkBundle,
            EngineOptions::default(),
        )
        .unwrap();
    ...
    let usage = service
        .usage(&catalog, &[OrbitCacheNamespace::ChunkBundle])
        .unwrap();
...
// was invisible to metadata checks; nanosecond mtimes plus orbit's
