...
//!
//! Every entry point takes an [`orbit_core::CleanupPlan`]; safe (ordinary)
...
use orbit_core::{CleanupAction, CleanupPlan, OrbitError, OrbitResult, now_iso8601};
use rusqlite::params;
...
/// responsibility of `orbit-cache` (Appendix A §12), driven by the same
...
impl<'a> CleanupExecutor<'a> {
    ...
    pub fn run_safe(&self, plan: &CleanupPlan) -> OrbitResult<CleanupOutcome> {
        ...
        match plan.action {
            ...
            _ => Err(OrbitError::CleanupWouldTouchPersistentData),
        }
    }
    ...
    pub fn run_reset_catalog(
        ...
        plan: &CleanupPlan,
        keep_settings: bool,
    ) -> OrbitResult<CleanupOutcome> {
        if plan.action != CleanupAction::ResetCatalog {
            return Err(OrbitError::Database(
    ...
    fn clear_expired_search_cache(&self) -> OrbitResult<CleanupOutcome> {
        let now = now_iso8601();
    ...
    fn clear_snippet_cache(&self) -> OrbitResult<CleanupOutcome> {
        let conn = self.catalog.lock();
        let deleted = conn
    ...
    fn remove_replaced_stale_indexes(&self) -> OrbitResult<CleanupOutcome> {
        let conn = self.catalog.lock();
        let deleted = conn
            .execute(
