...
fn seed_md_file(catalog: &Catalog, root: &std::path::Path, name: &str, content: &str)
    -> (orbok_core::SourceId, orbok_core::FileId)
{
    ...
    let src = sources.insert(NewSource {
    ...
    IndexJobRepository::new(catalog)
        .enqueue(JobType::Extract, Some(&src.source_id), Some(&record.file_id))
        .unwrap();
...
fn extract_chunk_index_and_search_roundtrip() {
    ...
    let succeeded = run_pending(&catalog, &extract, &chunk, 50).unwrap();
    assert!(succeeded >= 2, "expected extract + chunk jobs, got {succeeded}");

    let results = SearchService::new(&catalog).search("refresh tokens", 10).unwrap();
...
fn identifier_search_matches() {
    let dir = tempfile::tempdir().unwrap();
    let (catalog, cache) = setup(dir.path());
    seed_md_file(&catalog, dir.path(), "doc.md",
        "Error ERR-4042 is a critical system error.\n");
    let extract = ExtractionWorker::new(&catalog, &cache);
    let chunk = ChunkAndIndexWorker::new(&catalog, &cache);
    run_pending(&catalog, &extract, &chunk, 50).unwrap();
...
fn deleted_file_no_longer_matches() {
    let dir = tempfile::tempdir().unwrap();
    let (catalog, cache) = setup(dir.path());
    let (_, file_id) = seed_md_file(&catalog, dir.path(), "temp.md",
        "temporary secret phrase\n");
    let extract = ExtractionWorker::new(&catalog, &cache);
    let chunk = ChunkAndIndexWorker::new(&catalog, &cache);
    run_pending(&catalog, &extract, &chunk, 50).unwrap();
    assert!(!SearchService::new(&catalog).search("secret phrase", 10).unwrap().is_empty());
    ...
    let r = SearchService::new(&catalog).search("secret phrase", 10).unwrap();
    assert!(r.is_empty(), "deleted file chunks must not appear in results");
}
...
fn failed_rechunk_preserves_active_index() {
    let dir = tempfile::tempdir().unwrap();
    let (catalog, cache) = setup(dir.path());
    seed_md_file(&catalog, dir.path(), "stable.md",
        "the quick brown fox jumps over\n");
    let extract = ExtractionWorker::new(&catalog, &cache);
    let chunk = ChunkAndIndexWorker::new(&catalog, &cache);
    run_pending(&catalog, &extract, &chunk, 50).unwrap();

    let before = SearchService::new(&catalog).search("quick brown fox", 10).unwrap().len();
    assert!(before > 0);
    ...
    let after = SearchService::new(&catalog).search("quick brown fox", 10).unwrap().len();
    assert_eq!(before, after, "previous active index must survive a failed rechunk");
}
...
fn search_results_include_snippets() {
    let dir = tempfile::tempdir().unwrap();
    let (catalog, cache) = setup(dir.path());
    seed_md_file(&catalog, dir.path(), "notes.md",
        "# Notes\n\nImportant: token rotation deadline is Friday.\n");
    let extract = ExtractionWorker::new(&catalog, &cache);
    let chunk = ChunkAndIndexWorker::new(&catalog, &cache);
    run_pending(&catalog, &extract, &chunk, 50).unwrap();

    let results = SearchService::new(&catalog).search("token rotation", 10).unwrap();
    assert!(!results.is_empty());
    let snippet = results[0].snippet.as_deref().unwrap_or("");
    assert!(!snippet.is_empty(), "snippet should be loaded from source file");
}
