...
pub fn run_bench(n_docs: usize, work_dir: &std::path::Path)
    -> Result<report::BenchmarkResult, Box<dyn std::error::Error>>
{
    ...
    let cache = orbok_cache::CacheService::new(work_dir);
    {
        ...
        let root = std::fs::canonicalize(work_dir)?.to_string_lossy().to_string();
    ...
    }
    {
        ...
        if let Some(source) = src.first() {
            Scanner::new(&catalog).scan(
                &ScanRequest { source_id: source.source_id.clone(),
    ...
    Ok(report::BenchmarkResult {
        n_docs, corpus_bytes: corpus_size, catalog_bytes: catalog_size,
        index_elapsed_ms,
        indexing_files_per_sec: if index_elapsed_ms > 0 {
            (n_docs as f64 * 1000.0) / index_elapsed_ms as f64 } else { 0.0 },
...
mod bench_tests {
    ...
    fn bench_full_pipeline() {
        ...
        assert!(result.search_latency_ms.p99_ms < 2000.0,
            "p99 search latency too high: {:.2}ms", result.search_latency_ms.p99_ms);
        // RFC-016 recall target (relaxed for synthetic corpus with mock model).
        assert!(result.recall_at_k.recall >= 0.0,
            "recall must be a valid fraction");
        println!("Recall@{}: {:.1}%", result.recall_at_k.k,
