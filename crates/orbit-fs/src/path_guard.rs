...
//! [`OrbitError::PathOutsideSources`] — the guard never trusts
//! caller-provided paths (frontend or otherwise).

use crate::policy::CompiledPolicy;
use orbit_core::{HiddenFilePolicy, OrbitError, OrbitResult, SourceId, SymlinkPolicy};
use orbit_db::repo::SourceRecord;
...
impl PathGuard {
    ...
    /// does.
    pub fn canonicalize(path: &Path) -> OrbitResult<PathBuf> {
        std::fs::canonicalize(path)
            .map_err(|e| OrbitError::PathCanonicalization(format!("{}: {e}", path.display())))
    }
    ...
    pub fn validate(&self, requested: &Path) -> OrbitResult<ValidatedPath> {
        ...
        let source = self
            ...
            .ok_or(OrbitError::PathOutsideSources)?;
        ...
        if source.policy.symlink_policy == SymlinkPolicy::Ignore {
            ...
            if requested_inside && requested != canonical {
                ...
                if is_symlinked_below(&source.canonical_root, requested)? {
                    return Err(OrbitError::PolicyBlocked("symlink_policy_blocked"));
                }
            }
        }
        ...
        {
            return Err(OrbitError::PolicyBlocked("hidden_file_excluded"));
        }
        ...
        if let Ok(metadata) = std::fs::metadata(&canonical) {
            if metadata.is_file() && !source.policy.size_allowed(metadata.len()) {
                return Err(OrbitError::PolicyBlocked("file_too_large"));
            }
        }
...
fn is_symlinked_below(root: &Path, path: &Path) -> OrbitResult<bool> {
