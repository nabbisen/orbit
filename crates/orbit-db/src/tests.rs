//! Tests for orbit-db, validating the RFC-002 §12 testing requirements
...
fn new_file(src: &orbit_core::SourceId, path: &str) -> NewFile {
    NewFile {
        source_id: src.clone(),
        original_path: path.into(),
        canonical_path: path.into(),
...
fn catalog_persists_to_file() {
    ...
    let path = dir.path().join("orbit-catalog.sqlite3");
    {
