...
use orbit_core::{HiddenFilePolicy, OrbitError, SymlinkPolicy};
...
fn rejects_path_outside_sources() {
    ...
    assert!(matches!(err, OrbitError::PathOutsideSources));
}
...
fn ignore_policy_blocks_internal_symlink() {
    ...
    assert!(matches!(err, OrbitError::PolicyBlocked("symlink_policy_blocked")));
...
fn hidden_file_excluded_by_default() {
    ...
    assert!(matches!(err, OrbitError::PolicyBlocked("hidden_file_excluded")));
}
...
fn oversized_file_blocked() {
    ...
    assert!(matches!(err, OrbitError::PolicyBlocked("file_too_large")));
}
